function H = hPerfectChannelEstimate(pathGains,pathFilters,varargin)
%hPerfectChannelEstimate perfect channel estimation
%   H = hPerfectChannelEstimate(PATHGAINS,PATHFILTERS,NFFT,NCP,ACTIVEFFTIND)
...
%   (range 1:NFFT).
%
%   H = hPerfectChannelEstimate(...,OFFSET) performs perfect channel
...
%
%   H = hPerfectChannelEstimate(...,'OFDMSymbolOffset',SYMOFFSET)
...
if nargin>5
    if isnumeric(varargin{4})
        % hPerfectChannelEstimate(pathGains,pathFilters,FFTLen,CPLen,ActiveFFTInd,[Offset],[Name],[Value])
        offset = varargin{4};
        cpFraction = demodNVPairParse(varargin{5:end});
    else
        % hPerfectChannelEstimate(pathGains,pathFilters,FFTLen,CPLen,ActiveFFTInd,[Name],[Value])
        cpFraction = demodNVPairParse(varargin{4:end});
    end
else
    % hPerfectChannelEstimate(pathGains,pathFilters,FFTLen,CPLen,ActiveFFTInd)
