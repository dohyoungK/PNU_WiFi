function hVisualizeLibraryScenario(mapFileName,varargin)
...
%커버리지
if varargin{4}
    center = [varargin{3}(:,1) varargin{3}(:,2) varargin{3}(:,3)];
    radius = 25;
    n = 100;
    point = linspace(0, 2*pi, n);
    for i = 1:size(center,1)
        x = radius*cos(point) + center(i,1);
        y = radius*sin(point) + center(i,2);
        z = ones(1,n) * center(i,3);
        patch(x,y,z, 'yellow','FaceAlpha',.13)
    end
end
...
for n = 1:numel(HallNumber)
    floorPts = HallNumber(n).wall;
    floorPts(:,3) = [(floorPts(end,1)-1)*Height];
    floorPts(end,:) = [];

    v = floorPts;
    f = 1:size(v,1);
    if(n == 1)
        patch('Faces',f,'Vertices',v,'FaceColor','black','FaceAlpha',.13);
    end
    v(:,3) = v(:,3) + [Height];
    patch('Faces',f,'Vertices',v,'FaceColor','black','FaceAlpha',.13);
