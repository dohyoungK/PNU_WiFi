function W = hTGaxMMSEFilter(varargin)
%hTGaxMMSEFilter MMSE linear filter weights
%   W = hTGaxMMSEFilter(H,V,P,N0) returns linear MMSE receive filter given
%   the channel matrix and precoding matrix used at the transmitter.
%
%   W is a Nst-by-Nsts-by-Nlinks matrix containing the MMSE coefficients.
...
%   antennas.
%
%   V is a Nst-by-Nsts-by-Nt-by-Nlinks array containing the precoding
%   matrix (spatial mapping + cyclic shift).
%
%   P is a vector of length Nlinks containing receiver power in Watts.
%
%   N0 is the receiver noise power in Watts. It is assumed to be the same
%   for all links.
%
%   W = hTGaxMMSEFilter(HE,P,N0) returns linear MMSE receive filter where HE
...
%   W = hTGaxMMSEFilter(H,V,db2pow(P),db2pow(N0));
%
...
if nargin==4
    % hTGaxMMSEFilter(H,V,P,N0)
    % H is a Nst-by-Ntx-by-Nrx-by-Nlinks array containing the channel
    % response
    % V is a Nst-by-Nsts-by-Ntx-by-Nlinks array containing the precoding
    % matrix (spatial mapping + cyclic shift)
    % N0 is the noise variance
    [H,V,P,N0] = varargin{:};
    [Nst,Ntx,Nrx,Nlinks] = size(H);
    [Nst_w,Nsts,Ntx_w,Nlinks_w] = size(V);
    ...
    Pp = permute(V,[1 4 3 2]); % Nst-by-Nlinks-by-Ntx-by-Nsts
